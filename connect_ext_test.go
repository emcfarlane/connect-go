...
func TestServer(t *testing.T) {
	...
	// Add middleware to test HTTP level bindings.
	mux.Handle(pingRoute, http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) { // Check Content-Length is set correctly./
		...
		case pingv1connect.PingServicePingProcedure,
			...
			if request.ContentLength < 0 {
				t.Log(request.URL.Path)
				t.Errorf("expected Content-Length >= 0, got %d", request.ContentLength)
			}
		default:
			if request.ContentLength > 0 {
				t.Log(request.URL.Path)
				t.Errorf("expected Content-Length -1 or 0, got %d", request.ContentLength)
			}
		}

...
func TestPipeReadClose2(t *testing.T) {
	...
	go func() {
		time.Sleep(1 * time.Millisecond)
		pipe.CloseWithError(io.EOF) // delayClose
		done <- 0
	}()
...
func TestPipeWriteClose2(t *testing.T) {
	...
	msg := messagePayload{Data: make([]byte, 64)}
	n, err := pipe.Send(msg)
	<-done
	...
	t.Run("Write", func(t *testing.T) {
		...
		for i := 0; i < count; i++ {
			go func() {
				...
				msg := messagePayload{Data: []byte(input)}
				if n, err := pipe.Send(msg); n != len(input) || err != nil {
...
func BenchmarkPipe(b *testing.B) {
	for _, size := range []int{1, 512, 1024, 1024 * 1024} {
		...
		b.Run(fmt.Sprintf("messagePipeWriteTo:size=%d", size), func(b *testing.B) {
			b.ReportAllocs()
			pipe := &messagePipe{}
			go func() {
				for i := 0; i < b.N; i++ {
			...
			assert.Nil(b, err)
			assert.Equal(b, size*b.N, int(n))
			assert.Equal(b, size*b.N, dst.Len())
		})
	}
}
